...
class ControlEngine:
    ...
    def __isHome(self):
        """判断是否在主界面"""
        if get_img_box('../template/setting.png', box=(2000, 0, 560, 200)) is not None:
            return True
        else:
            return False
    ...
    def goHome(self):
        ...
        while True:
            if self.__isHome():
                break
            # 识图方式返回主界面
            # pos1 = get_img_box('./template/home.png', box=(2000, 0, 560, 150))
            # if pos1 is not None:
            #     auto_click(pyautogui.center(pos1), 1)
            # pos2 = get_img_box('./template/home2.png', box=(2000, 0, 560, 150))
            # if pos2 is not None:
            #     auto_click(pyautogui.center(pos2), 1)
    ...
    def saveRecordImg(self, name='Default') -> int:
        ...
        for filename in dirlist:
            print("CE 清理文件：", os.path.join(abspath, filename))
